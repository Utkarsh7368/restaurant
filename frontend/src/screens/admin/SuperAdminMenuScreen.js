...
export default function SuperAdminMenuScreen() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingDish) {
        await axios.put(`${API_URL}/superadmin/menu/${editingDish._id}`, payload, {
          headers: { Authorization: `Bearer ${token}` }
        });
      } else {
        await axios.post(`${API_URL}/superadmin/menu`, payload, {
          headers: { Authorization: `Bearer ${token}` }
        });
      }
      fetchDishes();
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`${API_URL}/superadmin/menu/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchDishes();
    } catch(e) { 
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <View style={styles.header}>
        <View>
          <Text style={styles.title}>Global Menu</Text>
          <Text style={styles.subtitle}>SuperAdmin Management</Text>
        </View>
      ...
      <Modal animationType="fade" transparent={true} visible={deleteModalVisible} onRequestClose={() => setDeleteModalVisible(false)}>
        <View style={styles.confirmOverlay}>
          <View style={styles.confirmBox}>
            ...
            <Text style={styles.confirmText}>Do you want to delete this dish across all branches?</Text>
...
const styles = StyleSheet.create({
  ...
  addBtn: { backgroundColor: PRIMARY, width: 48, height: 48, borderRadius: 16, justifyContent: 'center', alignItems: 'center', elevation: 6 },
  
  catHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 16, backgroundColor: '#fff' },
  ...
  card: { flexDirection: 'row', backgroundColor: '#fff', borderRadius: 16, padding: 12, marginBottom: 10, alignItems: 'center', borderWidth: 1, borderColor: '#f0f0f0', elevation: 1 },
  ...
  saveBtn: { backgroundColor: PRIMARY, borderRadius: 18, paddingVertical: 18, alignItems: 'center', marginTop: 35 },
  ...
  imagePickerContainer: { backgroundColor: '#f8fafc', borderRadius: 20, borderWidth: 2, borderColor: '#edf2f7', borderStyle: 'dashed', height: 140, justifyContent: 'center', alignItems: 'center', overflow: 'hidden' },
  ...
  confirmText: { fontSize: 16, color: '#666', textAlign: 'center', marginBottom: 30 },
